    ...
    private fun parseInstruction(currentTok: Token) = when (currentTok) {
        is LocalValueToken -> {
            ...
            when (instruction.string) {
                ...
                Load.NAME           -> parseLoad(currentTok)
                Call.NAME           -> parseCall(currentTok)
                SignExtend.NAME     -> parseSext(currentTok)
                ZeroExtend.NAME     -> parseZext(currentTok)
                Truncate.NAME       -> parseTrunc(currentTok)
                Flag2Int.NAME       -> parseFlag2Int(currentTok)
                ...
                Phi.NAME            -> parsePhi(currentTok)
                GetElementPtr.NAME  -> parseGep(currentTok)
                Neg.NAME            -> parseNeg(currentTok)
                Not.NAME            -> parseNot(currentTok)
                IntCompare.NAME     -> parseIcmp(currentTok)
                FloatCompare.NAME   -> parseFcmp(currentTok)
                GetFieldPtr.NAME    -> parseGfp(currentTok)
                Select.NAME         -> parseSelect(currentTok)
