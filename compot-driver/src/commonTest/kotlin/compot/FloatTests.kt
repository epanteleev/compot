package compot

import common.CommonCTest
import kotlin.test.Ignore
import kotlin.test.Test
import kotlin.test.assertEquals

abstract class FloatTests: CommonCTest() {
    @Test
    fun test1() {
        val result = runCTest("compot/float/float1", listOf(), options())
        assertEquals("f: .000000", result.output)
        assertEquals(0, result.exitCode)
    }

    @Test
    fun test2() {
        val result = runCTest("compot/float/float2", listOf(), options())
        assertEquals(0, result.exitCode)
    }

    @Test
    fun test3() {
        val result = runCTest("compot/float/float3", listOf(), options())
        assertEquals("nan\n", result.output)
        assertEquals(1, result.exitCode)
    }

    @Test
    fun test4() {
        val result = runCTest("compot/float/float4", listOf(), options())
        assertEquals("000000f87f\n", result.output)
        assertEquals(0, result.exitCode)
    }

    @Test
    fun test5() {
        val result = runCTest("compot/float/float5", listOf(), options())
        assertEquals("nan\n", result.output)
        assertEquals(1, result.exitCode)
    }

    @Test
    fun test6() {
        val result = runCTest("compot/float/float6", listOf(), options())
        assertEquals(1, result.exitCode)
    }

    @Test
    fun test7() {
        val result = runCTest("compot/float/float7", listOf(), options())
        assertEquals("-nan\n", result.output)
        assertEquals(1, result.exitCode)
    }

    @Test
    fun test8() {
        val result = runCTest("compot/float/float8", listOf(), options())
        assertEquals(1, result.exitCode)
    }
}

class FloatTestsO0: FloatTests() {
    override fun options(): List<String> = listOf()
}

class FloatTestsO1: FloatTests() {
    override fun options(): List<String> = listOf("-O1")
}