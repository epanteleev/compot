...
bool hideset_contains(Hideset *hs, char *s, int len) {
  for (; hs; hs = hs->next)
...
Token *new_str_token(char *str, Token *tmpl) {
...
Macro *add_macro(char *name, bool is_objlike, Token *body) {
...
MacroParam *read_macro_params(Token **rest, Token *tok, char **va_args_name) {
  MacroParam head = {};
  MacroParam *cur = &head;
...
MacroArg* read_macro_args(Token **rest, Token *tok, MacroParam *params, char *va_args_name) {
  Token *start = tok;
  tok = tok->next->next;
...
char *join_tokens(Token *tok, Token *end) {
...
char *search_include_next(char *filename) {
...
char *read_include_filename(Token **rest, Token *tok, bool *is_dquote) {
...
//   ...
//   #endif
char *detect_include_guard(Token *tok) {
