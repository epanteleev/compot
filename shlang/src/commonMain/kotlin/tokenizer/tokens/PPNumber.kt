...
class PPNumber(private val data: String, private val radix: Int, position: Position): CToken(position) {
    private var cachedNumber: Number? = null
    ...
    fun toNumberOrNull(): Number? {
        ...
        cachedNumber = when {
            ...
            data.endsWith("u")   -> data.substring(0, data.length - 1).toULongOrNull(radix)?.toLong()
            else -> data.toByteOrNull(radix) ?: data.toIntOrNull(radix) ?: data.toLongOrNull(radix) ?: data.toULongOrNull(radix)?.toLong() ?: data.toDoubleOrNull()
        }
        if (cachedNumber != null) {
            return cachedNumber
        }
