...
interface Address: Operand {
    companion object {
        fun mem(base: GPRegister, offset: Long, size: Int): Address {
            return Address2(base, offset, size)
        }

        fun mem(base: GPRegister, offset: Long, index: GPRegister, disp: Long, size: Int): Address {
            return Address4(base, offset, index, disp, size)
        }

        fun mem(label: String, size: Int): Address {
            return AddressLiteral(label, size)
        }
    }
}

open class Address2 internal constructor(open val base: GPRegister, open val offset: Long, override val size: Int): Address {
    override fun toString(): String {
        return if (offset == 0L) {
            "($base)"
        } else {
...
class Address4 internal constructor(val base: GPRegister, val offset: Long, val index: GPRegister, val disp: Long, override val size: Int):
    Address {
    override fun toString(): String {
        return if (offset == 0L) {
            "($base, $index, $disp)"
...
class ArgumentSlot(val base: GPRegister, val offset: Long, override val size: Int): Address {
    override fun toString(): String {
        return if (offset == 0L) {
            "($base)"
        } else {
    ...
    override fun invoke(size: Int): Operand {
        return ArgumentSlot(base, offset, size)
    }
}
